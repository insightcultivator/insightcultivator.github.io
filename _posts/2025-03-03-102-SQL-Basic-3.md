...
- 특정 연산이나 데이터를 반환하는 재사용 가능한 SQL 함수
    ...
    CREATE FUNCTION GetStudentAge(student_id INT) 
    ...
    -- 함수 호출 예제
    SELECT GetStudentAge(1) AS Alice_Age;
    ```
...
### **9.8 IF-THEN-ELSE-END IF 문**
- 조건문을 사용하여 특정 조건에 따라 SQL 문을 실행
    ```sql
    DELIMITER //

    CREATE PROCEDURE CheckAge(IN student_id INT)
    BEGIN
        DECLARE student_age INT;
        ...
        IF student_age >= 21 THEN
            SELECT '성인입니다.' AS message;
        ELSE
            SELECT '미성년자입니다.' AS message;
        END IF;
    END //
